/**
 * DigiDukaan Master Dashboard — v6 (fully dynamic dimensions).
 *
 * Every dimension list is built by SORT(UNIQUE(FILTER(...))) directly from
 * RawData, so when you paste fresh data:
 *   - New SELLERS auto-appear in the Distributor block
 *   - New CATEGORIES auto-appear in the Category block
 *   - New BRANDS auto-appear in the Brand block
 *   - New DATES auto-appear in the Date-wise block (sorted latest first)
 *
 * Requires Excel 365 / 2021 / Excel Online (UNIQUE / SORT / FILTER).
 *
 * RawData helper range capped at N=1200 (current ~1047 + headroom). Cuts
 * formula volume so Excel Online recalcs cleanly in the browser.
 */
...
// Cells that hold dynamic-array (spill) formulas. Excel 365 needs these flagged
// with cm="1" in the OOXML so it knows to enable auto-spill.
const DYNAMIC_ARRAY_CELLS = ['A10', 'I10', 'A74', 'A108'];

const SRC = 'C:\\Users\\Lenovo\\Downloads\\Digidukaan Dashboard.xlsx';
const DST = 'C:\\Users\\Lenovo\\Downloads\\Digidukaan Dashboard - v6.xlsx';

// Excel 365 dynamic-array functions must be stored with these prefixes in the
// XML — without them, Excel rejects the formula on open as a corruption.
...
async function main() {
  ...
  wb.creator = 'Digidukaan Builder v6 (fully dynamic)';
  ...
  // Buyer-distinct logic keys off buyer_mobile (column F) — name has too many
  // case-variants. Status-filtered (AK,AL,AM,AN,AO,AP,AQ,AR,AS,AT) exclude
  // CANCELLED; unfiltered (AW,AX,AY,AZ,BA) include everything for the date
  // table.
  const helperHeaders = {
  ...
  // ────────────────────── Dashboard ──────────────────────
  const widths = [26, 13, 13, 14, 14, 12, 12, 18, 12, 8, 9, 8, 10, 10, 9, 9, 15];
  for (let i = 0; i < widths.length; i++) dash.getColumn(i + 1).width = widths[i];

  // Title row 1
  dash.mergeCells('A1:Q1');
  ...
  function makeKpi(labelRange, valueRange, label, value, numFmt) {
    ...
    lc.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: COLOR.kpiBg } };
    ...
    vc.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: COLOR.kpiVal } };
  ...
  makeKpi('A5:B5', 'A6:B6', 'TOTAL VALUE',
    `=SUMIFS(${R_AV},${R_STATUS},"<>CANCELLED")`, '"Rs. "#,##0');
  makeKpi('C5:D5', 'C6:D6', 'AOV (Avg Order Value)',
    `=IFERROR(SUMIFS(${R_AV},${R_STATUS},"<>CANCELLED")/SUM(${R_AK}),0)`, '"Rs. "#,##0');
  makeKpi('E5:F5', 'E6:F6', 'DATE RANGE',
    `=IFERROR(IF(MAX(${R_AU})=0,"",TEXT(MAX(${R_AU})-$A$4+1,"dd-mmm")&" - "&TEXT(MAX(${R_AU}),"dd-mmm-yy")),"")`, '@');
  makeKpi('G5:H5', 'G6:H6', 'BRANDS LIVE', `=SUM(${R_AS})`, '#,##0');

  dash.getRow(7).height = 14;

  // ── DISTRIBUTOR BLOCK (A-G) — dimension via UNIQUE/SORT/FILTER ──
  const DIST_TITLE = 8;
  const DIST_HDR   = 9;
  const DIST_START = 10;
  const DIST_CAP   = 20;          // 20-row capacity
  const DIST_END   = DIST_START + DIST_CAP - 1;  // 29
  const DIST_TOTAL = DIST_END + 1;                // 30
  ...
  // A10: dynamic seller list. Plain _xlfn.UNIQUE — the SORT/FILTER/IFERROR
  // wrappers I tried earlier broke the spill. Excel will include one blank
  // entry for unused helper rows; the per-row IF(A_r="","",...) hides it.
  dash.getCell(`A${DIST_START}`).value = { formula:
    `${UNIQUE_FN}(${R_SELLER})` };

  // B-G per row formulas; they reference A_r (which is filled by the spill)
  ...
  // ── DATE-WISE BLOCK (I-Q) — dimensions dynamic from parsed_date ──
  const DATE_TITLE = 8;
  const DATE_HDR   = 9;
  const DATE_START = 10;
  const DATE_CAP   = 60;
  const DATE_END   = DATE_START + DATE_CAP - 1;  // 69
  const DATE_TOTAL = DATE_END + 1;                // 70

  dash.mergeCells(`I${DATE_TITLE}:Q${DATE_TITLE}`);
  sectionHeader(dash.getCell(`I${DATE_TITLE}`), '   DATE-WISE BREAKDOWN');
  dash.getRow(DATE_TITLE).height = 24;
  const dateHdrs = ['Date', 'Sellers', 'Orders', 'Open', 'Delivered', 'Cancelled', 'Returned', 'Buyers', 'Value (Rs.)'];
  for (let i = 0; i < dateHdrs.length; i++) columnHeader(dash.getRow(DATE_HDR).getCell(9 + i), dateHdrs[i]);
  dash.getRow(DATE_HDR).height = 30;

  // I10: dynamic date list. Plain UNIQUE on parsed_date helper column. Dates
  // appear in the order they first show up in RawData — since the source
  // export is sorted latest-first, the spill is descending naturally.
  dash.getCell(`I${DATE_START}`).value = { formula:
    `${UNIQUE_FN}(${R_AU})` };
  ...
  for (let r = DATE_START; r <= DATE_END; r++) {
    const iRef = `I${r}`;
    const blank = `${iRef}=""`;
    const openF = OPEN_STATUSES.map(s => `SUMIFS(${R_AW},${R_AU},${iRef},${R_STATUS},"${s}")`).join('+');
    ...
    dash.getCell(`L${r}`).value = { formula: `IF(${blank},"",${openF})` };
    ...
    dash.getCell(`Q${r}`).value = { formula: `IF(${blank},"",SUMIFS(${R_AV},${R_AU},${iRef}))` };
  ...
  for (let ci = 0; ci < 9; ci++) styleTotalRow(dash.getRow(DATE_TOTAL).getCell(9 + ci));
  ...
  dash.getCell(`Q${DATE_TOTAL}`).value = { formula: `SUM(${R_AV})` };
  ...
  // ── CATEGORY BLOCK (starts below max of dist/date totals) ──
  const blockStart = Math.max(DIST_TOTAL, DATE_TOTAL) + 2;  // 72
  ...
  // ── FREQUENCY BLOCK (4 fixed buckets) ──
  ...
  dash.mergeCells(`A${foot}:Q${foot}`);
  const fc = dash.getCell(`A${foot}`);
  fc.value = 'Notes: All blocks auto-update from RawData. Dates, sellers, categories, brands all appear automatically when new rows are added. KPIs and aggregates exclude CANCELLED orders; the DATE-WISE table includes all statuses to match SQL reporting. Cell A4 (Days of Ops) drives the frequency buckets and date range.';
  fc.font = { italic: true, size: 9, color: { argb: 'FF666666' } };
  fc.alignment = { vertical: 'middle', horizontal: 'left', indent: 1, wrapText: true };
  dash.getRow(foot).height = 48;
  ...
  // ExcelJS doesn't emit any of the OOXML required for dynamic-array spill.
  // To make UNIQUE/SORT/FILTER actually spill, the file needs:
  //   1. cm="1" attribute on each spill anchor cell
  //   2. xl/metadata.xml defining a "XLDAPR" metadata kind
  //   3. [Content_Types].xml Override for metadata.xml
  //   4. xl/workbook.xml.rels relationship from workbook → metadata
  //   5. xl/workbook.xml extLst referencing the metadata relationship
  const zip = await JSZip.loadAsync(fs.readFileSync(DST));

  // (1) cm="1" on each dynamic-array anchor
  ...
  // (2) xl/metadata.xml — declares the XLDAPR (dynamic array) metadata kind
...
<metadata xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:xda="http://schemas.microsoft.com/office/spreadsheetml/2017/dynamicarray" xmlns:xlrd="http://schemas.microsoft.com/office/spreadsheetml/2017/richdata"><metadataTypes count="1"><metadataType name="XLDAPR" minSupportedVersion="120000" copy="1" pasteAll="1" pasteValues="1" merge="1" splitFirst="1" rowColShift="1" clearFormats="1" clearComments="1" assign="1" coerce="1"/></metadataTypes><futureMetadata name="XLDAPR" count="1"><bk><extLst><ext uri="{bdbb8cdc-fa1e-496e-a857-3c3f30c029c3}"><xda:dynamicArrayProperties fDynamic="1" fCollapsed="0"/></ext></extLst></bk></futureMetadata><cellMetadata count="1"><bk><rc t="1" v="0"/></bk></cellMetadata></metadata>`;
  ...
  // (3) [Content_Types].xml — add Override for metadata
  ...
  // (4) xl/_rels/workbook.xml.rels — add relationship
  ...
  console.log(`Helper range: rows 2..${N} (${N-1} rows × 17 helpers = ${(N-1)*17} formulas)`);
