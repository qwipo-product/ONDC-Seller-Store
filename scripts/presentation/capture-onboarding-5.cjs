// Capture the SKU Details -> Price & Inventory tab (Step 9).
...
(async () => {
  ...
  for (const b of await page.$$('button, [role="tab"]')) {
    ...
    if (/price & inventory/i.test(t)) { await b.click(); break; }
  }
  await sleep(1200);
  await page.screenshot({ path: path.join(OUT, "step9-price-inventory-tab.png") });
  console.log("captured step9-price-inventory-tab");
  await browser.close();
  console.log("done");
})();
